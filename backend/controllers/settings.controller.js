...
exports.addSettings = async (req, res) => {
  try {
    const settingsData = await Settings.create(req.body);

    res.status(201).json({ data: settingsData });
  } catch (error) {
...
exports.getAll = async (req, res) => {
  try {
    ...
    res.status(200).json({ data: settingsData });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
exports.getOne = async (req, res) => {
  try {
    ...
    const settingsData = await Settings.findByPk(id);
